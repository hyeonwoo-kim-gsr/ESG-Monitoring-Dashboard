(function () {
  ...
  // 원본 수집 스크립트의 COMPETITORS 목록과 동일한 순서로 유지하세요.
  var COMPANIES = [
  ...
  /* 타임존 영향 없는 순수 날짜 연산 (YYYY-MM-DD 문자열 기준) */
  ...
  function clipHtml(article) {
    var cls = sentimentClass(article.sentiment);
    var stamp = article.related_count > 1
      ? '<span class="stamp">동일 주제 보도 ' + article.related_count + '건</span>'
      : "";
    return (
      '<div class="clip ' + cls + '">' +
        '<div class="clip-top">' +
          '<a class="clip-title" href="' + escapeHtml(article.link) + '" target="_blank" rel="noopener">' + escapeHtml(article.title) + "</a>" +
          '<span class="clip-date">' + escapeHtml(article.date) + "</span>" +
        "</div>" +
        (article.description ? '<p class="clip-desc">' + escapeHtml(truncate(article.description, 140)) + "</p>" : "") +
        '<div class="clip-foot">' +
          '<span class="clip-company-tag">' + escapeHtml(article.company) + " · " + sentimentLabel(article.sentiment) + "</span>" +
          stamp +
        "</div>" +
      "</div>"
    );
  ...
  function renderSentimentSections(articles, opts) {
    ...
    ["POSITIVE", "NEGATIVE"].forEach(function (senti) {
      ...
      html += '<div class="section-heading ' + sentimentClass(senti) + '">' +
        ...
        ' <span style="font-family:var(--font-mono); font-size:12px; font-weight:400; color:var(--muted);">(' + list.length + "건)</span></div>";
      ...
      if (list.length === 0) {
        ...
        COMPANIES.forEach(function (company) {
          ...
          arr.forEach(function (a) { html += clipHtml(a); });
          html += "</div>";
        });
      } else {
        list.sort(function (a, b) { return a.date < b.date ? 1 : -1; });
        list.forEach(function (a) { html += clipHtml(a); });
      }
      html += "</div>";
    });
    return html;
  ...
  function renderWeekly(startStr, endStr) {
    ...
    Promise.all(months.map(loadMonth)).then(function (arrays) {
      ...
      el.innerHTML = ranked.map(function (a, idx) {
        return '<div class="rank-row"><div class="rank-badge">' + (idx + 1) + "</div>" + clipHtml(a) + "</div>";
      }).join("");
    });
  }
  ...
  function renderSearch(keyword) {
    ...
    loadAllMonths().then(function (all) {
      ...
      el.innerHTML = '<p class="hint">총 ' + matched.length + "건 검색됨</p>" + matched.map(clipHtml).join("");
    });
  }
